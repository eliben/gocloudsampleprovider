...
func newHarness(ctx context.Context, t *testing.T) (drivertest.Harness, error) {
	dir := filepath.Join(os.TempDir(), "go-cloud-gocloudsampleprovider")
...
func TestNewBucket(t *testing.T) {
	t.Run("BucketDirMissing", func(t *testing.T) {
		dir, err := ioutil.TempDir("", "gocloudsampleprovider")
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("BucketIsFile", func(t *testing.T) {
		f, err := ioutil.TempFile("", "gocloudsampleprovider")
		if err != nil {
			t.Fatal(err)
		}
...
func (verifyPathError) ErrorCheck(b *blob.Bucket, err error) error {
	...
	wantSuffix := filepath.Join("go-cloud-gocloudsampleprovider", "key-does-not-exist")
...
func TestOpenBucketFromURL(t *testing.T) {
	dir := filepath.Join(os.TempDir(), "gocloudsampleprovider")
	if err := os.MkdirAll(dir, os.ModePerm); err != nil {
		t.Fatal(err)
	}
	...
	tests := []struct {
		...
		{"gcspfile:///bucket-not-found", "myfile.txt", true, false, ""},
		// File doesn't exist -> error at read time.
		{"gcspfile://" + dirpath, "filenotfound.txt", false, true, ""},
		// OK.
		{"gcspfile://" + dirpath, "myfile.txt", false, false, "hello world"},
		// OK, host is ignored.
		{"gcspfile://localhost" + dirpath, "myfile.txt", false, false, "hello world"},
		// Invalid query parameter.
		{"gcspfile://" + dirpath + "?param=value", "myfile.txt", true, false, ""},
	}
